...
bool PEGParser::Context::Visit(SymbolType _type, const char*& _p, vector<Symbol>& _v)
{
...
namespace
{
	...
	if (r)
	{
		for (;;)
		{
			...
			if (r)
			{
				r = _ctx.Visit(SymbolType_Range, p, v);
			}
			if (!r)
			{
				p = b;
		...
		if (r)
		{
			...
			case SymbolType_Definition:
			{
				if (_ctx.fail[SymbolType_Definition].count(pBegin))
					return false;
	r = _ctx.Visit(SymbolType_Identifier, p, v);
	if (r)
	{
	...
	if (r)
	{
		r = _ctx.Visit(SymbolType_LEFTARROW, p, v);
		if (r)
		{
			r = _ctx.Visit(SymbolType_Expression, p, v);
		}
	}
				if (r)
			...
			case SymbolType_Expression:
			{
				if (_ctx.fail[SymbolType_Expression].count(pBegin))
					return false;
	r = _ctx.Visit(SymbolType_Sequence, p, v);
	if (r)
	{
		for (;;)
		{
	...
	if (r)
	{
		r = _ctx.Visit(SymbolType_Definition, p, v);
		if (r)
		{
			for (;;)
			{
	...
	if (r)
	{
		r = _ctx.Visit(SymbolType_Primary, p, v);
		if (r)
		{
	...
	if (r)
	{
		for (;;)
		{
			...
			if (r)
			{
				r = _ctx.Visit(SymbolType_Char, p, v);
			}
			if (!r)
			{
				p = b2;
	...
	if (!r)
	{
		...
		if (!r)
		{
			p = b;
			v.erase(v.begin() + s, v.end());
			r = _ctx.Visit(SymbolType_LITERAL, p, v);
			if (!r)
			{
				p = b;
				v.erase(v.begin() + s, v.end());
				r = _ctx.Visit(SymbolType_CLASS, p, v);
				if (!r)
				{
					p = b;
					v.erase(v.begin() + s, v.end());
	...
	for (;;)
	{
		const char* b = p;
		size_t s = v.size();
		r = _ctx.Visit(SymbolType_Item, p, v);
		if (!r)
		{
			p = b;
			v.erase(v.begin() + s, v.end());
