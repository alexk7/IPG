...
namespace
{
	...
	struct Private
	{
		...
		static Node* Traverse_Class_1(Node* p0, PTNodeChildren& v)
		{
			...
			if (p0)
			{
				p0 = p0->end.find(PTNodeType_Range)->second;
				if (p0)
		...
		static Node* Traverse_Definition(Node* p0, PTNodeChildren& v)
		{
			...
			if (!p1)
				return 0;
			p0 = p0->end.find(PTNodeType_Identifier)->second;
			if (p0)
			...
			if (p0)
			{
				...
				if (p0)
				{
					p0 = p0->end.find(PTNodeType_LEFTARROW)->second;
					if (p0)
						v.push_back(PTNodeChild(PTNodeType_LEFTARROW, p0));
					if (p0)
					{
						p0 = p0->end.find(PTNodeType_Expression)->second;
						if (p0)
		...
		static Node* Traverse_Expression(Node* p0, PTNodeChildren& v)
		{
			...
			if (!p1)
				return 0;
			p0 = p0->end.find(PTNodeType_Sequence)->second;
			if (p0)
			...
			if (p0)
			{
				for (;;)
				{
					Node* p2 = Traverse_Expression_1(p0, v);
					if (!p2)
						break;
					p0 = p2;
				}
		...
		static Node* Traverse_Grammar(Node* p0, PTNodeChildren& v)
		{
			...
			if (p0)
			{
				p0 = p0->end.find(PTNodeType_Definition)->second;
				if (p0)
		...
		static Node* Traverse_Literal_1_1(Node* p0, PTNodeChildren& v)
		{
			...
			if (p0)
			{
				p0 = p0->end.find(PTNodeType_Char)->second;
				if (p0)
		...
		static Node* Traverse_Prefix(Node* p0, PTNodeChildren& v)
		{
			...
			if (p0)
			{
				p0 = p0->end.find(PTNodeType_Suffix)->second;
				if (p0)
					v.push_back(PTNodeChild(PTNodeType_Suffix, p0));
			}
			return p1;
		}
		...
		static Node* Traverse_Primary_1(Node* p0, PTNodeChildren& v)
		{
			...
			if (p0)
			{
				...
				if (p0)
				{
					Node* p2 = p0->end.find(PTNodeType_LEFTARROW)->second;
					if (p2)
						v.push_back(PTNodeChild(PTNodeType_LEFTARROW, p0));
					if (p2)
						p0 = 0;
		...
		static Node* Traverse_Suffix(Node* p0, PTNodeChildren& v)
		{
			...
			if (!p1)
				return 0;
			p0 = p0->end.find(PTNodeType_Primary)->second;
			if (p0)
				v.push_back(PTNodeChild(PTNodeType_Primary, p0));
			if (p0)
			{
				Node* p2 = p0;
				p0 = Parse_QUESTION(p2);
				if (!p0)
				{
					p0 = Parse_STAR(p2);
					if (!p0)
